...
type StoreUnit = u16;
type Store = BitBox<Lsb0, StoreUnit>;
...
mod tests {
  ...
  #[test]
  fn new_l_size_1() {
    assert_eq!(BV::new(bitbox![Lsb0, u16; 0]).large_sum.len(), 1);
  }
  #[test]
  fn new_l_size_large() {
    assert_eq!(BV::new(bitbox![Lsb0, u16; 0; BV::LARGE]).large_sum.len(), 1);
  }
  #[test]
  fn new_l_size_large_plus_1() {
    assert_eq!(BV::new(bitbox![Lsb0, u16; 1; BV::LARGE + 1]).large_sum.len(), 2);
  }
  #[test]
  fn new_large_sum() {
    assert_eq!(BV::new(bitbox![Lsb0, u16; 1; BV::LARGE + 1]).large_sum, vec![0, 1024]);
  }
  #[test]
  fn new_small_sum() {
    let bv = BV::new((0 .. 50).map(|i| i < 20).collect::<BitVec<Lsb0, u16>>().into());
    assert_eq!(bv.small_sum, vec![0, 16, 20, 20]);
  }
  #[test]
  fn rank() {
    let bv = BV::new(bitbox![Lsb0, u16; 1; BV::LARGE + 1]);
